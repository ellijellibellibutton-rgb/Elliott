...
/**
 * Populates default settings + demo data. Idempotent: every section checks
 * its own row count first, so calling this again after partial or full
 * seeding only fills in what's still missing.
 */
export async function seedDatabase(prisma: PrismaClient) {
  ...
  if (!existingAdmin) {
    ...
    log.push("Created admin account (password: AAPLE2026)");
  }
  ...
  if (!existingSettings) {
    ...
    log.push("Created campaign settings");
  }
  ...
  if (categoryCount === 0) {
    ...
    log.push(`Created ${scoringDefs.length} scoring categories`);
  }
  ...
  if (sprintCount === 0) {
    ...
    log.push(`Created ${sprintDefs.length} sprints`);
  }
  ...
  if (teamCount === 0) {
    ...
    log.push(`Created ${teamDefs.length} teams`);
  }
  ...
  if (studentCount === 0 && teams.length > 0) {
    ...
    log.push("Created demo students");
  }
  ...
  if (judgeCount === 0) {
    ...
    log.push(`Created ${judgeDefs.length} judges`);
  }
  ...
  if (prizeCount === 0) {
    ...
    log.push(`Created ${prizeDefs.length} prizes`);
  }
  ...
  if (awardCount === 0) {
    ...
    log.push(`Created ${awardDefs.length} awards`);
  }
  ...
  if (activityCount === 0 && teams.length > 0 && categories.length > 0) {
    ...
    log.push(`Created ${activityRows.length} demo activities`);
  }
  ...
  if (winCount === 0 && awards.length > 0 && teams.length > 0 && allStudents.length > 0) {
    ...
    log.push("Created Win of the Week history");
  }
